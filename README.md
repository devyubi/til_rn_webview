...
## 2. 정보 전달 및 마켓 등록을 위한 추천 솔루션

- 공통적으로 IOS, Android 개발 가능
- Flutter : 구글의 `Dart` 언어로 개발
- React Native : React, HTML, css, ts, 자체 component(내장되어있음) 로 개발 (개념상 하이브리드)
...
- 최신 버전은 절대로 배제함. (안정화 버전을 사용.)
- https://velog.io/@it-ju/React-native-cli-개발환경-세팅하기
- https://reactnative.dev/docs/set-up-your-environment

### 4.1. choco 설치 및 환경 확인

- 윈도우 검색 → Power Shell 관리자 모드로 실행 ( 마우스 오른쪽 클릭 )
- https://chocolatey.org/install
- 아래 문장을 Power Shell 에 입력 후 Enter
...
### 5.4. Android SDK Location 을 복사해 둠
...
## 6. Window 에서 JDK 를 인식하도록 `Path` 를 설정함

### 6.1. `환경변수설정` 진입

- 윈도우 하단 툴바에서 `시스템 환경 변수 편집` 입력 후 검색 (띄워쓰기 주의)
- `시스템 환경 윈도우` 하단 → `환경변수...` 버튼 클릭
- `시스템 변수` 목록 → `새로 만들기...`
- `변수이름` 항목 : `ANDROID_HOME` (오타 꼭 주의)
- `변수값` 항목 : `C:\Users\user\AppData\Local\Android\Sdk` (오타 꼭 주의!!)
- 확인 버튼으로 등록
